...
// decision that made /register-showroom admin-only-reachable.
...
// The two tests below actually submit the form and so share
// registerShowroomPublicAction's own IP-based rate-limit ledger
// (showroom_registration_attempts) — grouped so a beforeAll can clear
// that ledger first, so neither starts against a quota already exhausted
// by a previous run/session from the same local IP. Deliberately NOT
// serial: each submits exactly once with its own unique email, so even in
// the worst case of both running concurrently in different workers (each
// running this same beforeAll, harmlessly redundant), the combined
// attempt count from this file alone (2) stays well under both the
// per-IP (3/hour) and per-email (2/day, and each email here is unique
// anyway) limits — serial execution isn't needed for correctness, and
// avoids the masking problem where one test's failure (e.g. the happy
// path, dependent on the shared Mailtrap sandbox actually having quota
// left) would otherwise skip the other, unrelated test after it.
test.describe("form submission", () => {
  test.beforeAll(async () => {
    await admin().from("showroom_registration_attempts").delete().neq("email", "");
  });
